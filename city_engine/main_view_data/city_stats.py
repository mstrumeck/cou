...
class CityStatsCenter:
    def __init__(self, city, data):
        ...
        self.raw_water_production = (
            self.raw_water_stats.calculate_raw_water_production_in_city()
        )
        self.raw_water_allocation = (
            self.raw_water_stats.calculate_raw_water_allocation_in_city()
        )

        self.clean_water_bilans = self.clean_water_stats.clean_city_water_bilans()
        self.clean_water_production = (
            self.clean_water_stats.calculate_clean_water_production_in_city()
        )
        self.clean_water_allocation = (
            self.clean_water_stats.calculate_clean_water_allocated_in_city()
        )

        self.building_under_construction = (
            self.building_stats.list_of_buildings_under_construction()
        )
...
class CityEnergyStats:
    ...
    def city_energy_bilans(self):
        return (
            self.calculate_energy_production_in_city()
            - self.calculate_energy_allocation_in_city()
        )
...
class CityRawWaterStats:
    ...
    def raw_city_water_bilans(self):
        return (
            self.calculate_raw_water_production_in_city()
            - self.calculate_raw_water_allocation_in_city()
        )
...
class CityCleanWaterStats:
    ...
    def clean_city_water_bilans(self):
        return (
            self.calculate_clean_water_production_in_city()
            - self.calculate_clean_water_allocated_in_city()
        )
...
class CityBuildingStats:
    ...
    def home_areas_demand(self):
        return "{}/{}".format(
            sum(
                self.data.list_of_workplaces[h].max_employees
                - self.data.list_of_workplaces[h].people_in_charge
    ...
    def list_of_buildings_under_construction(self):
        return [
            [b.name, b.current_build_time, b.build_time]
            for b in self.data.list_of_buildings
            if b.if_under_construction is True
        ]
