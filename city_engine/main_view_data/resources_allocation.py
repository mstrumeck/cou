...
class ResourceAllocation:
    ...
    def resource_allocation(self, dataset):
        for provider_ob in dataset["list_of_source"]:
            source_keys = [
                dataset["list_without_source"][b].row_col_cor
            ...
            provider_total_production = provider_ob.total_production(
                self.data.list_of_workplaces, self.data.citizens_in_city
            )
            guard = 0
            while getattr(
                provider_ob, allocated_resource
            ) < provider_total_production and guard < len(
                dataset["list_without_source"]
            ):
                key = pattern[guard]
