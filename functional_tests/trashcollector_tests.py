...
class TrashCollectorTest(BaseTest):
    ...
    def test_garbage_management(self):
        ...
        self.assertEqual(dumping_ground.current_employees, 0)
        ...
        dumping_ground = DumpingGround.objects.latest('id')
        dust_cart = DustCart.objects.latest('id')
        self.assertEqual(dumping_ground.current_employees, 5)
        self.assertGreater(dumping_ground.current_space_for_trash, 100)
        self.assertEqual(dust_cart.current_employees, 3)
        self.assertEqual(CityField.objects.filter(city=self.city_one).aggregate(Sum('pollution'))['pollution__sum'], 10)
