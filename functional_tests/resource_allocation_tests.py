...
class ResourceAllocationTest(BaseTest):
    def test_resource_allocation(self):
        ...
        SewageWorks.objects.create(
            city=self.city, city_field=CityField.objects.get(id=cf_id)
        )
        WaterTower.objects.create(
            city=self.city, city_field=CityField.objects.get(id=cf_id - 1)
        )
        ...
        DumpingGround.objects.create(
            city=self.city, city_field=CityField.objects.get(id=cf_id - 3)
        )
        WindPlant.objects.create(
            city=self.city, city_field=CityField.objects.get(id=cf_id - 4)
        )
        WindPlant.objects.create(
            city=self.city, city_field=CityField.objects.get(id=cf_id - 5)
        )
        ...
        self.assertEqual(
            WindPlant.objects.filter(city=self.city_one).aggregate(
                Sum("energy_allocated")
            )["energy_allocated__sum"],
            0,
        )
        self.assertEqual(
            WaterTower.objects.filter(city=self.city_one).aggregate(
                Sum("raw_water_allocated")
            )["raw_water_allocated__sum"],
            0,
        )
        self.assertEqual(
            SewageWorks.objects.filter(city=self.city_one).aggregate(
