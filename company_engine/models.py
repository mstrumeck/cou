...
class Company(models.Model):
        ...
        for good_type in GOODS_TO_PRODUCT:
            for material in materials:
                data.list_of_workplaces[self].add_new_good(
                    good_type, material.size, self._get_quality_of_product(material.quality, data), material.price)
            ...
            if component_type in data.market.resources:
                for component in data.market.resources[component_type].instances:
                    if self.cash >= component.price:
                        temp_comp = component_type.objects.create(size=0, quality=component.quality,
    ...
    def _get_productivity(self, data):
        productivity = [self._get_energy_productivity(data), self._get_water_productivity(data)]
    ...
    def _get_quality(self, data):
        ...
        for e_cat, e_cat_needed in zip(employee_categories, employee_categories_needed):
            employees = self._get_employee_by_appendix(data.list_of_workplaces, data.citizens_in_city, e_cat)
            if employees:
