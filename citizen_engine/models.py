...
class Citizen(models.Model):
    ...
    workplace_content_type = models.ForeignKey(
        ContentType,
    ...
    workplace_object = GenericForeignKey(
        "workplace_content_type", "workplace_object_id"
    )

    resident_content_type = models.ForeignKey(
        ContentType, on_delete=models.SET_NULL, null=True, related_name="resident_place"
    )
    ...
    school_content_type = models.ForeignKey(
        ContentType, on_delete=models.SET_NULL, null=True, related_name="school_place"
    )
    ...
    def change_work_for_better(self, workplaces, citizens, save_instance):
        ...
        self.__increase_num_of_vacancies_in_dataset(
            workplaces,
            self.workplace_object,
            citizens[self].current_profession.education,
        )
    ...
    def grab_job(
        self, possible_workplaces, workplaces, edu_level, citizens, save_instance
    ):
        best = self.__find_better_work_option(possible_workplaces)
        self.workplace_object = best
        self.__decrease_num_of_vacancies_in_dataset(workplaces, best, edu_level)
    ...
    def find_work(self, workplaces, citizens, save_instance):
        ...
        if possible_workplaces is None:
            ...
            self.grab_job(
                possible_workplaces, workplaces, edu_level, citizens, save_instance
            )

    def __decrease_num_of_vacancies_in_dataset(
        self, workplaces, workplace_key, edu_level
    ):
        setattr(
            workplaces[workplace_key],
            Education.MATRIX[edu_level],
            getattr(workplaces[workplace_key], Education.MATRIX[edu_level]) - 1,
        )

    def __increase_num_of_vacancies_in_dataset(
        self, workplaces, workplace_key, edu_level
    ):
        setattr(
            workplaces[workplace_key],
    ...
    def _possible_workplaces(self, workplaces):
        for position in self.__possible_position()[::-1]:
            if [
                w
                for w in workplaces
                if getattr(workplaces[w], Education.MATRIX[position])
            ]:
                return (
                    [
                        w
    ...
    def __find_better_work_option(self, candidates):
        if len(candidates) == 1:
            ...
            pattern = sorted(
                candidates,
                key=lambda x: (abs(x.city_field.col - col), abs(x.city_field.row - row))
                if isinstance(x, BuldingsWithWorkes)
                else (random.randrange(HEX_NUM_IN_ROW), random.randrange(ROW_NUM)),
            )
            chances = {
                len(pattern) / rating: building
                for rating, building in enumerate(pattern, start=1)
            }
    ...
    def __possible_position(self):
        ...
        if self.edu_title != "None":
            return [
                x for x in degrees if degrees.index(self.edu_title) >= degrees.index(x)
            ]
        else:
            return [ELEMENTARY]
...
class Profession(models.Model):
    ...
    job_grade_info = {
        job_grade[0]: {"divisor": divisor, "border": border}
        for job_grade, divisor, border in zip(
    ...
    def __check_job_grade(self):
        if self.proficiency > Profession.job_grade_info[self.job_grade]["border"]:
            cur_level_index = [
                job_grade[0] for job_grade in Profession.JOB_GRADES
            ].index(self.job_grade)
            try:
    ...
    def update_proficiency(self, citizen):
        ...
        if citizen_education:
            citizen.current_profession.proficiency += (
                sum(citizen_education) / len(citizen_education)
            ) / self.__proficiency_divisor()
        else:
            citizen.current_profession.proficiency += (
                0.01 / self.__proficiency_divisor()
            )
...
class Education(models.Model):
    MATRIX = {
        ELEMENTARY: "elementary_vacancies",
        COLLEGE: "college_vacancies",
        PHD: "phd_vacancies",
