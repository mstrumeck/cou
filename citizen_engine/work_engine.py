...
class CitizenWorkEngine:
    ...
    def assign_job_for_people_without_education(self):
        if [w for w in self.active_workplaces if self.active_workplaces[w].elementary_vacancies]:
            workplaces_without_qualifications = [w for w in self.active_workplaces
                                                 if self.active_workplaces[w].elementary_vacancies]
            for e in (u for u in self.data.citizens_in_city if u.workplace_object is None
                                                               and u.resident_object is not None
                                                               and u.age >= 18
                                                               and u.edu_title == 'None'):
                e.grab_job(
