...
from city_engine.models import BuldingsWithWorkes, TradeDistrict, DumpingGround, DustCart
...
class DataContainersWithEmployees(ABC):
    ...
    def _create_data_for_building_with_worker(self, instance, citizens, citizens_in_city):
        self.workers_costs = 0
        self.max_employees = sum([instance.elementary_employee_needed,
                                  instance.college_employee_needed,
                                  instance.phd_employee_needed])

        self.elementary_employees = [e for e in citizens
        ...
        self.elementary_vacancies = instance.elementary_employee_needed - len(
            [e for e in citizens if e.workplace_object == instance
             and citizens_in_city[e].current_profession.education == ELEMENTARY])

        self.college_employees = [e for e in citizens
                                  if e.workplace_object == instance
        ...
        self.phd_employees = [e for e in citizens
                              if e.workplace_object == instance
    ...
    def _create_data_for_dumping_ground(self, bi, citizens, citizens_in_city, vehicles):
        ...
        if DustCart.objects.filter(dumping_ground=bi).exists:
            for dc in DustCart.objects.filter(dumping_ground=bi):
                vehicles[dc] = VehicleDataContainer(instance=dc, citizens=citizens, citizens_in_city=citizens_in_city)
    ...
    def _create_data_for_trade_district(self, bi):
        ...
        for model in [model for model in apps.get_app_config('company_engine').get_models()
        ...
        if good_type in self.goods:
            for good_in_company in self.goods[good_type].instances:
                if good_in_company.quality == quality:
                    ...
                    return
            self.goods[good_type].instances.append(good_type.objects.create(size=size,
                                                                               quality=quality,
                                                                               price=price,
                                                                               company=self.ci))
        else:
            self.goods[good_type] = ResourcesDataContainer([good_type.objects.create(size=size,
                                                                               quality=quality,
                                                                               price=price,
                                                                               company=self.ci)])
    ...
    def create_goods_produced_by_company(self):
        for good in self.goods_to_product:
            if good.objects.filter(company=self.ci).exists():
                self.goods[good] = ResourcesDataContainer(instances_list=list(good.objects.filter(company=self.ci)))
...
class MarketDataContainer:
    ...
    def create_resources_in_market(self, instance):
        subclasses = (model for model in apps.get_app_config('resources').get_models()
                      if issubclass(model, MarketResource) and model is not MarketResource)
        for sub in subclasses:
            if sub.objects.filter(market=instance).exists():
                self.resources[sub] = ResourcesDataContainer(list(sub.objects.filter(market=instance)))
        ...
        if resource_type in self.resources:
            for resource_in_market in self.resources[resource_type].instances:
                if quality == resource_in_market.quality:
                    ...
                    resource_in_market.price = self._calculate_new_price(resource_in_market, size, price)
                    return
            self.resources[resource_type].instances.append(resource_type.objects.create(size=size,
                                                                               quality=quality,
                                                                               price=price,
                                                                               market=market))
        else:
            self.resources[resource_type] = ResourcesDataContainer([resource_type.objects.create(size=size,
                                                                               quality=quality,
                                                                               price=price,
                                                                               market=market)])
...
class ResidentialDataContainer:
    ...
    def __create_data_for_residential(self, fields_data):
        ...
        pollution_penalty = basic_rent * (fields_data[self.bi.city_field].pollution / 100) \
            if fields_data[self.bi.city_field].pollution else 0
...
class BuildingDataContainer(DataContainersWithEmployees):
    ...
    def semafor(self, citizens, citizens_data, vehicles):
        ...
        if isinstance(self.bi, DumpingGround):
            self._create_data_for_dumping_ground(self.bi, citizens, citizens_data, vehicles)
